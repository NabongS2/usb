...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		Statement stmt = null;
		// sql을 DB에 전달해 수행하고
		// 결과(ResultSet 또는 DML 성공 행의 개수)를 반환 받는데 사용되는 객체
		...
		try {
			...
			String ip = "localhost";
			// localhost == 127.0.0.1 (look back ip == 자기 컴퓨터 주소)
			...
			String user = "kh_lnh"; // 계정명
			
			String pw = "oracle_lnh123A"; // 비밀번호
			
			// DriverManager 객체
			// - 메모리에 로드된 JDBC드라이버 객체를 이용해 DB에 접근 하고
			...
			while(rs.next()) {
				...
				// rs.next()
				// - ResultSet 객체는 조회 결과를 한 번에 테이블로 다루는 것이 아닌
...
//				java.sql.Date hireDate = rs.getDate("HIRE_DATE");
			...
			// SQLException : JDBC 관련 객체 이용 중 발생되는 예외의 최상의 부모
