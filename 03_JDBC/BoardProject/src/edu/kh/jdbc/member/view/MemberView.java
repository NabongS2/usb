...
public class MemberView {
	...
	public void memberMenu() {
		...
		do {
			...
			try {
				...
				System.out.println("4. 비밀 번호 변경(현재 비밀번호, 새 비밀번호, 새 비밀번호 확인 ");
	...
	private void updatePassword() {
		...
		String newPass,newPassCorr = null;
		while(true) {
			// 새 비밀번호 입력
			...
			// 같을 때 까지 무한 반복
			if(newPass.equals(newPassCorr)) break;
		...
		try {
			...
			// -> 성공하면 1 / 실패하면 0 -> int 형 변수
			int result = service.updatePassword(pass,newPass,Session.loginMember.getMemberNO());
			...
			Session.loginMember.setMemberPw(newPass);
