...
public class CarSerive {
	...
	public void ex1() {
		...
		// 부모 참조 변수 = 자식 객체
		Car c2 = new Truck(); // Truck 안에 있는 Car만 참조! 자식 -> 부모 업캐스팅
	...
	public void ex3() {
		...
		for(Car c: carList) {
			...
			// Light car일 경우 -> 할인율 : 30%
			// Car일 경우 -> 차종이 등록 되어 있지 않습니다.
			
			/* instanceof 연산자 */
			// - 참조하는 객체의 타입을 검사하는 연산자
			
			// ex) 참조 변수 a instanceof 클래스명
			// -> 참조변수의 탑입과 클래스의 타입이 일치하면
			//    클래스의 타입이 일피하면 true, 아니면 false
			
			
			if(c instanceof Truck) {
				// c가 참조하는 변수가 Truck 인 경우
			
				System.out.println("최대 적재하중 : " 
										+ ((Truck)c).getWeight());
				...
				// 해결 방법 :
				// instanceof 를 통해 참조하는 객체의 자료형을 판단하여
				// 적절한 형식으로 다운 캐스팅 진행
				...
				System.out.println("차종이 등록 되지 않았습니다.");
			}
	...
	public void printCar(Car c) {
		...
		// 동적 바인딩
		// Car c = new Truck이면 Car만 참고 해야하는데 Truck에도 toString 이 오버라이딩 되어있음으로 Car로 안가고 Truck으로 방향을 틀음
		// - "프로그램 실행 중"
