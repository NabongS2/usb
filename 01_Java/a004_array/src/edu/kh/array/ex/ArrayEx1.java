...
public class ArrayEx1 {
	...
	public void ex1() {
		...
		// 배열 선언
		int[] arr; 
		// - int[] 배열을 참조할 공간을 할당 하고 이를 arr이라고 부르겠다.
		
		// * 기본 자료형 8개를 제외한 나머지는 "참조형"이라고 한다!
		
		// 배열 할당
		arr = new int[4];
		// - 새롭게 int형 변수 4개짜리 배열은 생성하고
		//   이를 arr에 대입
		//        -> arr이 해당 배열을 참조
	...
	public void ex2() {
		...
		// 2) Heap 영역에 새롭게 int 4칸 짜리 배열을 할당
		// -> index 번호 추가, 기본값 (0)으로 초기화
		// 	+ 시작주소 부여 ex) 0x5123
		
		// 3) Heap 영역에 생선된 배열의 시작 주소 (0x5123)를
...
//		String[] menuArr = new String[10];
	...
	// 배열 사용법 6
		public void ex6() {
			// 인원수를 입력 받아 그 크기만큼 정수 배열 을 선언 및 할당하고
			...
			for(int i=0; i<scoreArr.length; i++) {
				...
				if(scoreArr[i]<min) {
					// 기존 min(최소값)보다 현재 배열 요소의 값이 더 큰 경우
					min = scoreArr[i];
					minIndex = i; // 최저점 인덱스
				}
			}
...
//			}
			...
			//System.arraycopy(원본 배열명, 원본 배열 복사 시작 인덱스,
							// 복사 배열명, 복사하는 배열의 삽입 시작 인덱스, 복사 길이)
