...
public class ListService {
	...
	public void ex1() {
		...
		/* 컬렉션 특징
		 * 1) 크기 제약이 없다.
		 * 2) 추가, 삭제, 수정 등의 기능이 구현 되어있다.
		 * 3) 여러 타입의 객체를 저장 할 수 있다.
		...
		ArrayList list = new ArrayList(3); // 매개변수 생성자 -> 3칸 짜리 생성
		
		// -> 예상되는 데이터 수에 따라 생성자를 골라서 사용
		// -> 생성자에 따라 속도, 메모리 효율이 달라짐
		
		// 메모리 용량을 아끼기 위해 어차피 늘어나니까 하지만 용량이 늘어나는 것도 로직이기 때문에 큰 용량이 필요하면 애초에 1000 설정 해놓기
		...
		for(int i=0; i<list.size(); i++) {
			...
			else if(list.get(i) instanceof Double) { // 요소가 Integer인 경우
	...
	public void ex2() {
		...
		// String으로 타입이 제한 된 ArrayList 객체를 생성하여
		// 부모 타입인 List 참조
		...
		for(int i=0; i<list.size();i++) {
			String str = list.get(i); // 다운 캐스팅 필요가 없다
