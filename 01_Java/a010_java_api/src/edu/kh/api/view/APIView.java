...
public class APIView {
	...
	private void ex1() {
		...
		if(service.addStudent2(grade, classRoom, number, name)) {
			...
			System.out.println("중복되는 학생이 존재하거나, 가득 찼습니다.");
		}
...
//		System.out.println(s1.hashCode()); // 필드를 가지고 만든 수
//		System.out.println(s2.hashCode());
	...
	private void ex5() {
		...
		// StringBuffer.capacity : 현재 버퍼의 크기 16
		System.out.println(sb.capacity());
		System.out.println("현재 주소 : "+System.identityHashCode(sb)); // 현재 주소 : 157627094
		...
		// - StirngBffer에 저장된 문자열을 String 형태로 반환
		System.out.println("sb에 저장된 문자열 : "+sb.toString()); // abc
		System.out.println("abc 추가 후 주소 : "+System.identityHashCode(sb)); // 157627094 가변성 (객체는 유지 저장되는 값만 바뀐다.)
		
		sb.append("bef");
		System.out.println("sb에 저장된 문자열 : "+sb.toString()); // abcbef
		System.out.println("def 추가 후 주소 : "+System.identityHashCode(sb)); // 157627094
		...
		sb.insert(2, "@");
		System.out.println(sb.toString()); // ab@cbef

		// delete(int start, int end) : 삭제 (start 이상 end 미만) 보통 미만으로 끝나는 경우가 대다수
		sb.delete(3, 5); // 3, 4 삭제
	...
	private void ex7() {
		 ...
		 *  2) 기본 자료형을 객체로 다뤄야하는 경우가 있기 때문에
		 *     (컬렉션)
		 * 
		 * * /
		 */
		...
		double d = Double.parseDouble("3.1415923151326")+10.0;
		 ...
		 * - 사용자가 신경쓰지 않아도
		 *   상황에 따라 기본 자료형 <-> Wrapper Class 객체로 변하는 기술
		 *   자동으로 변하는 기술
		 * */
