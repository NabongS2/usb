...
*** JS 정규 표현식 객체 생성 방법 ***
1. const regEx = new RegExp("정규 표현식");
2. const regEx = /정규 표현식/;
...
2. regEx.exec(문자열)
-> 문자열이 정규 표현식 패턴에 부합하면 첫 번째 매칭되는 문자열을 반환
    없으면 null 반환
...
// 특징
// 1) 원하는  value.를 얻어오는 방법
// - 객체명.key / 객체명["key"]

// 2) 객체에 특정 K가 존재하지 않으면 추가 할 수 있다.
// - const obj = {"a":1 , "b":2};
// obj.c=3 -> {"a":1 , "b":2, "c":3};
...
memberEmail.addEventListener("input",()=>{
    ...
    // 2) 입력 받은 이메일과 정규식 일치 여부 확인
    if(regEx.test(memberEmail.value)){ // 유효한 경우

        // ♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡♥♡
       /* fetch() API를 활용한 ajax(비동기 통신) */ 
       // GET 방식 ajax 요청 파라미터는 쿼리 스트링으로...
        ...
        .then(count =>{
            if(count==0){
                ...
                emailMessage.classList.remove("error"); // .error 스타일 적용
...
document.getElementById("signUpFrm").addEventListener("submit",e=>{
    // checkObj에 모든 value가 true 인지 검사
    // (배열용 for문)
    // for ... of : 향상된 for문
    ...
    if(regEx.test(memberPw.value)){
        ...
        if(memberPwConfirm.value.trim().length==0){
            ...
            if(memberPw.value==memberPwConfirm.value){
                ...
                pwMessage.innerText="비밀번호가 일치 않습니다.";
...
memberNickname.addEventListener("input",()=>{
    ...
    if(memberNickname.value.trim() ==''){
        nickMessage.innerText="한글, 숫자, 영어로만 2~10글자";
    ...
    if(regEx.test(memberNickname.value)){ // 유효
        ...
        fetch("/dupCheck/nickname?ninknam="+memberNickname.value)
        .then(resp => resp.text()) // 데이터를 text로 파싱(변환)
        .then(count=>{
            if(count==0){ // 중복 아닌 경우
