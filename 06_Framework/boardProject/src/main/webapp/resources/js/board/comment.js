...
function selectCommentList(){
    ...
    // 자원을 이름(주소)으로 구분(REpresentaional)하여 자원의 상태(State)를 주고 받는 것(Transfer)

    // -> 주소를 명시하고 HttpMethod(GET, POST, PUT, DELETE)를 이용해
    // 지정된 자원에 대한 CRUD 진행
    ...
    fetch("/comment?boardNo="+boardNo) // GET 방식 주소에 파라미터에 담아 전달
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    fetch("/comment",{
        ...
        body : JSON.stringify(data) // JS 객체를 -> JSON 파싱
    })
    .then(resp => resp.text())
    .then(result => {
        if(result > 0){ // 등록 성공
