...
public class MemberController {
	...
	// -> MemberService를 구현한 MemberServiceImpl의 Bean이 주입
	@Autowired
...
//	@RequestMapping(value="/login", method=RequestMethod.POST)
	public String login(HttpServletRequest req) {
		// 파라미터 전달 방법 1 : HttpServeltRequest를 이용하는 방법
		// -> Controller 메서드에 매개변수로 HttpServeltRequest를 작성
		// 매개변수에 적으면 왜 사용 가능할까?
		// Spring Framwork가 제공하는
		// Arqument Resolever (매개변수 해결사)가 해결해줘서
...
//	@PostMapping("/login")
	public String login(/*@RequestParam("inputEmail")*/ String inputEmail,
		...
		// 파라미터 name 속성 값과 매개 변수 명이 같으면 @RequestParam 생략 가능
...
//	@PostMapping("/login")
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// 파라미터 전달 방법 3 : @ModelAtrribute를 이용한 방법
		
		// - DTO(또는 VO)와 같이 사용하는 어노테이션
		// - 전달 받은 파라미터의 name 속성 값이 같이 사용되는 DTO의 필드명과 같다면
		//   자동으로 setter 을 호출해서 필드에 값을 세팅
	...
	public String login(Member inputMember, Model model
		...
		// 매개 변수 String refere에 저장
		
		// Model model : 데이터 전달용 객체
		// 데이터를 K : V 형식으로 담아 전달
		// 기본적으로 request scope
		// @SessionAttributes 어노테이션과 함꼐 사용시 Session scope
		...
		if(loginMember != null){ // 로그인 성공
			...
			// Servlet -> HttpSession.setAtribute(key,value);
			...
			/*  redirect(재요청) 시
			 * 기존 요청(request)이 사라지고 새로운 요청(request)을 만들게 되어
			 * 
			 * redirect 된 페이지에서는 이전 요청이 유지 되지 않는다!!
			 ...
			 * 
			 * RedirectAttributes : 리다이렉트 시 데이터를 request scope로 전달 할 수 있게 하는 객체
			 * 
			 * 응답 전 : request scope
			 * 
