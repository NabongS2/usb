...
// 함수 호출
testFn();
// num3 is not defined(정의 되지 않음 == 존재 하지 않거나 접근 불가능)
// console.log("함수 외부에서 선언된 (함)num3 : "+num3);

// JS 코드 중간에 에러가 발생하면
// 같은 파일/함수 내 이후 부분 코드가 해석되지 않는다.
console.log("함수 외부에서 선언된 (전)num4 : "+num4);

// 에러 발생
// console.log("함수 외부에서 (if)num5 : "+num5);
console.log("함수 외부에서 (if)num6 : "+num6);
...
function typeTest(){
    ...
    box.innerHTML += "<br>heiget : "+heiget +"/"+typeof heiget;
    ...
    // Java의 객체 --> class에 작성된 내용대로 메모리(heap)에 할당
    // JS의 객체 {K:V} Map 형식으로 작성
    // ** key 는 무조건 string으로 고정 **

    const user = {"id":"uesr01",'pw':'pass01',address:'서울시 중구'}
    box.innerHTML += "<br>user : "+user +"/"+typeof user; // [object Object]/object
...
function doubleFn(fn){ // 2배로 반환하는 함수
