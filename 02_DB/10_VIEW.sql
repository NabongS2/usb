...
GRANT CREATE VIEW TO kh_lnh;

--> 다시 kh 계정으로 접속 해서 VIEW 생성 구문 수행
...
-- INSERT 확인
SELECT * FROM V_DCOPY2;
--> VIEW는 값을 저장하지 않는다고 했는데 저장된 것 처럼 보인다!
--> 실제 VIEW에 저장된 것이 아니라
--  VIEW를 생성 할 때 사용한 서브쿼리의 
--  테이블(DEPT_COPY2)에 값이 삽입 되어있다!
...
---> 그런데 DEPT_TITLE 에는 NOT NULL 제약조건이 설정되어
--   NULL 값이 삽입될 수 없음.

-- ORA-01400: NULL을 ("KH_LNH"."DEPT_COPY2"."DEPT_TITLE") 
-- 안에 삽입할 수 없습니다  

-- 데이터 무결성 : 중복 X, NULL X -> 신뢰도 높은 데이터
--> 대부분의 컬럼에 NOT NULL 제약 조건 추가되어 있음
...
/* SEQUENCE (순서, 연속)
 * - 순차적으로 일정한 간격의 숫자(번호)를 발생 시키는 개체
 *   (번호 생성기)
 * 
 * *** SEQUENCE 왜 사용할까?? ***
 ...
 * 
 * PK가 지정된 컬럼에 삽입될 값을 생성 할 때 SEQUENCE를 이용하면 좋다!
 * 
 *   [작성법]
...
-- SEQ_TEST_NO의 증가 값을 1, 최대값 155로 수정
...
-- 시퀀스는 START WHIT 를 변경하고 싶을 때 삭제 후 다시 생성해야 한다.
 ...
 * 
 * - 인덱스도 하나의 객체이다 보니 별도 저장 공간이 필요 (메모리 사용)
 * 
 * - 인덱스 생성 시간이 필요함 
   ...
   ** 인덱스가 자동 생성되는 경우 **
   -> PK 또는 UINQUE 제약조건이 설정된 컬럼에 대해
   	  UNIQUE INDEX가 자동 생성 된다.
...
WHERE TEST_ID = 'TEST500000'; -- 28/23/20 ms

-- INDEX를 사용 O 검색
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; -- 9/1/0 ms
...
-- INDEX 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 1/0 ms
