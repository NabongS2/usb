...
* SELECT 컬럼명 | * (모든 컬럼)
* FROM 테이블명
* WHERE 조건식; -- 조건의 맞는 행만 조회
* 
*/

-- EMPLOEE 테이블에서 모든 행, 모든 칼럼을 조회
-- (WHERE절 없음 == 모든 행)
-- * : ALL, 모두, 전부
...
-- EMPLOYEE 테이블에서 
-- 급여가 300만 이상 500만 이하인 직원의 모든 정보 컬럼 조회
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 
-- 급여가 300만 이상 500만 이하가 아닌 직원의 모든 정보 컬럼 조회
-- == 300만 미만, 500만 초과
SELECT *
FROM EMPLOYEE
...
-- NOT IN (값1, 값2, 값3, ...) : () 안의 값을 제외한 TRUE
SELECT EMP_ID , EMP_NAME , DEPT_CODE 
FROM EMPLOYEE
...
-- IS NOT NULL : 컬럼 값이 NULL인 아닌 경우에 TRUE

-- EMPLOYEE 테이블에서 부서가 없는 사원의 모든 컬럼을 조회
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE IS NULL;

-- EMPLOYEE 테이블에서 부서가 있는 사원의 모든 컬럼을 조회
SELECT *
FROM EMPLOYEE
...
-- OR PHONE IS NULL; -- PHONE 이 빈칸인 사람도 출력
...
--> 문제점 :  와일드 카드(_)와 패턴에 사용된 일반 문자가
--			같은 문자 이기 때문에 구분이 안되는 문제가 발생

--> 해결 방법 : ESCAPE 옵션을 이용하여 
SELECT EMP_ID ,EMP_NAME ,EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___#_%' ESCAPE '#';
				--> '#' 뒤에 한글자(_) 일반 문자로 벗어나게 함
...
AND HIRE_DATE <='00/12/31';
--> '90/01/01' == 문자열 -> DATE 타입으로 변경
...
* FROM 테이블명
* WHERE 조건식
* ORDER BY 컬러명 | 별칭 | 컬럼 순서 [오름/내림 차순]
* 		[NULL FISRT | LAST]
*/
...
/*1*/FROM EMPLOYEE
/*3*/ORDER BY /* SALARY *12 */ /* 연봉 */ 2 DESC ; -- COLUMN 순서
				-- 컬럼명		:	  별칭   :	순서

/*(주의) WHERE절에서 별칭|순서는 사용할 수 없다.*/
/*3*/SELECT EMP_NAME, SALARY *12 AS 연봉
/*1*/FROM EMPLOYEE
/*2*/WHERE 연봉 >= 5000000; -- 오류 발생(SELECT절 해석이 안된 상태에서 별칭을 WHERE절 작성)
