...
-- 같은 테이블에 별칭을 EMP, MGR 지정해주고
-- EMP 테이블에서 사번, 이름, 사수사번을 가져오고 MGR에서 사수 이름을 가져온다.
...
-- EMPLOYEE 테이블 JOB_CODE CHAR(2) 직급코드
-- JOB 테이블의 JOB_CODE CHAR(2) 직급코드
-- 데이터 타입까지 같을 때 사용 가능
...
WHERE JOB_NAME ='대리'
-- JOB JOB_NAME = '대리' 인 사람을 찾아서
AND LOCAL_NAME LIKE 'ASIA%';     
-- LOCATION LOCAL_NAME 이 ASIA로 시작하는
...
AND SUBSTR(EMP_NAME,1,1) LIKE '전%';
...
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
--WHERE SUBSTR(DEPT_TITLE,1,4) LIKE '해외영업%'
...
--연봉에 보너스포인트를 적용하시오.
-- 연봉 = (급여 * (1+보너스)) *12
SELECT EMP_NAME, JOB_NAME, SALARY, (SALARY*(1+NVL(BONUS,0)))*12 "연봉(보너스 포함)"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
