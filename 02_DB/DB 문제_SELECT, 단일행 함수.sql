...
-- EXTRACT(DAY FROM LAST_DAY(HIRE_DATE))-EXTRACT(DAY FROM HIRE_DATE)+1 "입사한 달의 근무일수" 
FROM EMPLOYEE;
...
-- 음수 일 때는 TRUCK는 문제가 생김으로 FLOOR을 쓰는게 더 적합하다.
-- 날짜는 완벽한 365일이 아니기 때문에 날짜에서 날짜를 빼서 /365로 나누면 문제가 됨
FROM EMPLOYEE;
