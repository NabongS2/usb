...
FROM EMPLOYEE
WHERE DEPT_CODE ='D9'; -- 성동일, 송중기, 노옹철
...
--			(주의사항!) ORDER BY 절 해석 전에 먼저 행번호를 부여
--SELECT ROWNUM, EMP_ID 
--FROM EMPLOYEE
--ORDER BY SALARY; -- FROM -> SELECT -> ORDER BY 순서로 
...
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME ='대리'
-- > ANY 가장 작은 값보다 큰가 / < ANY 가장 큰 값보다 작은가
...
-- EXISTS : 서브 쿼리에서 조회된 결과와 일치하는 행이
...
SELECT EMP_ID , EMP_NAME , NVL(DEPT_TITLE,'소속없음') , JOB_NAME , MAIN.HIRE_DATE 
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON (DEPT_CODE=DEPT_ID)
...
WHERE HIRE_DATE
	= (SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB
	   /* WHERE ENT_YN  = 'N'
	   AND NVL(SUB.DEPT_CODE,'0') = NVL(MAIN.DEPT_CODE,'0') */
...
-- ** 인라인 뷰 사용 시 컬럼명에 유의 해야한다!
	--> 메인쿼리에서 인라인 뷰의 컬러명을 그대로 사용해야 한다!
-- 급여 평균이 590만인 부서를 조회
SELECT * FROM
	(SELECT DEPT_CODE, DEPT_TITLE , FLOOR(AVG(SALARY)) 평균
	FROM EMPLOYEE
