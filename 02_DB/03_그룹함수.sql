 ...
 * 6: ORDER BY 컬럼명|별칭|컬럼순서 정렬 방식 [NULLS FIRST|LAST]
 * 
 */
...
FROM EMPLOYEE
GROUP BY DEPT_CODE -- 7개의 그룹으로 만들어줘
ORDER BY DEPT_CODE;

-- 부서별 급여함, 급여평균, 인원 수, 최고참 입사일, 막내 입사일
-- 부서코드 오름차순 조회
SELECT DEPT_CODE,
	SUM(SALARY), ROUND(AVG(SALARY)), COUNT(*),
...
SELECT DEPT_CODE , AVG(SALARY) -- 4) DEPT_CODED와 급여 평균 조회 
FROM EMPLOYEE 				   -- 1) EMPLOYEE 테이블
WHERE DEPT_CODE IN ('D5','D6') -- 2) 부서코드가 'D5','D6'인 행만 추려서
GROUP BY DEPT_CODE; 		   -- 3) 추려진 결과 내에서 부서코드 별로 그룹을 지거
...
-- EMPLOYEE 테이블에서 부서별로 같은 직급인 사원의 급여 함을
-- 부서 코드 오름차순으로 조회
...
-- * HAVING 절 : 그룹함수로 조회할 그룹에 대한 조건을 설정 할 때 사용
...
/*3*/HAVING AVG(SALARY) >= 3000000 -- GROUP BY 묶인 그룹에 조건을 대입
/*5*/ORDER BY 1;

-- 부서별 급여 합이 900만원을 초과하는 부서의
-- 부서코드, 급여합, 인원 수 조회
...
-- CUBE : 그룹으로 지정된 모든 그룹에 대한 중간 집계와 전체 합계를 처리하는 함수
-- ROLLUP : 그룹 별로 중간 집계와 전체 합계를 처리하는 함수
SELECT JOB_CODE , SUM(SALARY) 
FROM EMPLOYEE
GROUP BY JOB_CODE
...
-- UNION ALL 합집합 + 교집합 확인
-- 부서 코드가 'D5' 또는 급여가 300만 초과인 사원  (중복 O)
SELECT EMP_NAME , EMP_NAME , DEPT_CODE , SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
...
-- MINUS 차집합
-- 부서 코드가 'D5' 이지만 급여가 300만 초과인 사원 제외 조회
SELECT EMP_NAME , EMP_NAME , DEPT_CODE , SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
