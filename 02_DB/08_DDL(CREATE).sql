...
VALUES ('MEM04','123456','이지연','911105-1123454');

--  데이터  삽입 확인
SELECT * FROM MEMBER;
COMMIT; --> TCL
		--> 트랙잭션에 저장된 DML 구문을 DB에 반영

ROLLBACK; --> 마지막 COMMIT 상태로 돌아감(트랙잭션에 저장된 DML 삭제)

-- 주민등록번호, 전화번호와 같은 숫자로만 이루어진 데이터를
-- MEMBER 타입으로 지정할 경우의 문제점

CREATE TABLE MEMBER2(
	MAMBER_NAME VARCHAR2(15),
	MEMBER_SSN NUMBER,
	MEMBER_PHONE NUMBER
);
...
-- ORA-01400: NULL을 ("KH_LNH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_LNH.SYS_C007462)에 위배됩니다
-- ORA-00001: 무결성 제약 조건(KH_LNH.USER_ID_UK)에 위배됩니다
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_LNH.USER_ID_NAME_UK)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_LNH.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_LNH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_LNH.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_LNH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--> 복합키로 PK가 설정되어 있어도
--  결국은 PK => NULL 삽입 불가능
...
-- 무결성 제약조건(KH_LNH.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 50 이라는 부모키 없음
...
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- 무결성 제약조건(KH_LNH.GRADE_CODE_FK1)이 위배되었습니다
--			- 자식 레코드(기록)가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_LNH.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(30) CONSTRAINT UK_PNO UNIQUE 
					 CONSTRAINT NN_PNO NOT NULL,
	GENDER VARCHAR2(10) CONSTRAINT CK_GENDER CHECK(GENDER IN ('남','여')),
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(50),
	STATUS VARCHAR2(10) CONSTRAINT CK_STATUS CHECK(STATUS IN ('Y','N'))
...
CREATE TABLE USER_TEST2(
	USER_NO NUMBER ,
	USER_ID VARCHAR2(20) ,
	USER_PWD VARCHAR2(30) CONSTRAINT NN_USER_PWD2 NOT NULL,
	PNO VARCHAR2(30) CONSTRAINT NN_PNO2 NOT NULL,
	GENDER VARCHAR2(10),
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(50),
	STATUS VARCHAR2(10) CONSTRAINT NN_STATUS2 NOT NULL
						
	,CONSTRAINT PK_USER_TEST2 PRIMARY KEY(USER_NO)
	,CONSTRAINT UK_USER_ID2 UNIQUE(USER_ID)
...
VALUES (1, 'ID01', 'PW01', '123456-1234567', '남', '010-1234-5678',  '서울시', 'Y');

INSERT INTO USER_TEST
VALUES (2, 'ID02', 'PW02', '234567-2345678', '여', '010-1234-1234',  '강원도', 'Y');

INSERT INTO USER_TEST
VALUES (3, 'ID03', 'PW03', '345678-3456789', '남', '010-5678-5678',  '인천광역시', 'N');

INSERT INTO USER_TEST
VALUES (4, 'ID04', 'PW04', '456789-4567891', '여', '010-4321-4321',  '경기도', 'N');

INSERT INTO USER_TEST
VALUES (5, 'ID05', 'PW05', '567891-5678912', '남', '010-4321-1234',  '서울특별시', 'Y');
...
AS (SELECT * FROM EMPLOYEE); -- 괄호 없어도 됨
--AS 서브쿼리; -- 서브쿼리의 결과(RESELT SET)의 형태로 테이블 생성
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT SAL_LEVEL
