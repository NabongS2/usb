...
-- 다른 프로그램이랑 작성 방법이 다 다르니 쓸 때 꼭 찾아서 보기
...
SELECT 123.456,
	ROUND(123.456), -- 소수점 첫째 자리에서 반올림 123
	ROUND(123.456,1), -- 소수점 첫째 자리까지 표현 123.5
	ROUND(123.456,2), -- 소수점 둘째 자리까지 표현 123.46
	ROUND(123.456,0), -- 소수점 0번째 자리까지 표현 123 -> 정수 안쓰는 거랑 같음
	ROUND(123.456,-1), -- 소수점 -1번째 자리까지 표현 -> 정수 1의 자리에서 반올림 120
	ROUND(123.456,-2) -- 정수 10 의 자리에서 반올림 100
FROM DUAL;

---------------------------------------------------
--TRUNC(숫자|컬럼 [,소수점 위치]) : 잘라내기, 버림, 절삭
...
-- 2023-03-07 12:23:22.000, 2023-03-07 12:23:22.520 +0900

-------------------------------------------------------
-- MONTH_BTWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이를 반환
SELECT '약'
...
/* 날짜 패턴 Y, R의 차이점 */
-- 년도를 짧게 해석하는 경우 
-- 50미만 : Y,R 둘다 앞부분에 현재세기(21C==2000년대)를 적용
...
-- DECODE(계산식|컬럼명, 조건1, 결과1, 조건2, 조건2... [,아무것도 일치 안할 때])
...
SELECT EMP_ID , EMP_NAME , SALARY,
	CASE
		...
		WHEN SALARY >= 3000000 THEN '중급' -- AND SALARY <5000000
		ELSE '초급'
	END
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6'
...
SELECT MAX(SALARY), MIN(SALARY), 
	   ...
	   MIN(HIRE_DATE) /*가장 늦게*/, MAX(HIRE_DATE) /*가장 최근에 입사한*/
FROM EMPLOYEE;
...
SELECT COUNT(*) FROM EMPLOYEE; -- 23 (모든 행의 개수)

-- EMPLOYEE 테이블에 부서 코드가 있는 사원의 수
SELECT COUNT(DEPT_CODE) -- 21 (NULL 제외)
FROM EMPLOYEE;

-- EMPLOYEE 테이블에 존재하는 직급 코드의 개수
SELECT COUNT(DISTINCT JOB_CODE) -- 7 (중복을 제외하고 카운드 )
FROM EMPLOYEE;
