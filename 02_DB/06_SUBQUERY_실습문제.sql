...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
				   FROM EMPLOYEE SUB
				   WHERE ENT_YN='N'
				   AND NVL(MAIN.DEPT_CODE,'0') = NVL(SUB.DEPT_CODE,'0'))
ORDER BY HIRE_DATE;
...
TO_CHAR((SALARY + (SALARY*NVL(BONUS,0)))*12,'L999,999,999') "보너스 포함연봉"
FROM EMPLOYEE MAIN
JOIN JOB J ON (MAIN.JOB_CODE= J.JOB_CODE)
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12)
			=(SELECT MAX(FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12))
			  FROM EMPLOYEE SUB
			  WHERE SUB.JOB_CODE = MAIN.JOB_CODE)
ORDER BY "만 나이" DESC;

					
SELECT FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12) FROM EMPLOYEE;
SELECT TO_CHAR((SALARY + (SALARY*NVL(BONUS,0)))*12,'L999,999,999') FROM EMPLOYEE;
