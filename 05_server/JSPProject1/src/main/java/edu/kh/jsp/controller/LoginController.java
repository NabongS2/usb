...
public class LoginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 일치 하지 않으면 "아이디 또는 비밀번호가 일치하지 않습니다." 출력
		...
		// --------------------------------------------------------
		// 기존에 req에 없던 값(message)을 속성(arribute)으로 추가 
		// -> Map 형식으로 작성
		req.setAttribute("message", message); // 변수이름, 변수값
		// --------------------------------------------------------
		...
		// 그 요청에 맞는 화면으로 변함(요청에 맞는 응답을 했기 때문에)
		
		// -> Servelt이 가지고 있던 req, resp 객체를 JSP에게 전달하여 요청을 실제로 위임
